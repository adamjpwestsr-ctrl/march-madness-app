...
export default function NascarSchedulePanel() {
  ...
  const handleUpload = async () => {
    ...
    const res = await fetch("/api/nascar/schedule", {
      method: "POST",
      body: formData,
    });
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
      <h2 className="text-xl font-semibold text-white mb-4">📅 Import NASCAR Season Schedule</h2>

      {/* File Upload */}
      <div className="mb-4">
        <label className="text-slate-300 block mb-2">Upload Schedule CSV</label>
        <input
          type="file"
          accept=".csv"
      ...
      {status === "error" && (
        <p className="text-red-400 mt-4 font-semibold">
          Error importing schedule. Check file format.
        </p>
      )}
    </section>
  );
