  ...
  return (
    <div className="relative w-full overflow-hidden py-2 min-h-[40px] bg-slate-900/60 border-t border-b border-slate-800 backdrop-blur group">
      ...
      {marqueeGames.length === 0 ? (
        <div className="text-slate-500 text-sm px-6">No recent or live scores available.</div>
      ) : (
        <div className="flex items-center gap-8 whitespace-nowrap animate-ticker group-hover:[animation-play-state:paused]">
          {marqueeGames.map((game: any) => {
            ...
            const sportKey = Object.keys(SPORTS).find((s) =>
              game?.league?.slug?.includes(SPORTS[s as keyof typeof SPORTS].path)
            ...
            return (
              <div key={`${game.id}-${Math.random()}`} className="flex items-center gap-3 px-6 text-sm text-slate-300">
                <span className="text-xl">{icon}</span>

                {away?.team?.logo && (
                  <img src={away.team.logo} alt={away.team.displayName} className="w-6 h-6 rounded-full object-cover" />
                )}
                ...
                {home?.team?.logo && (
                  <img src={home.team.logo} alt={home.team.displayName} className="w-6 h-6 rounded-full object-cover" />
                )}
                ...
                {isFinal && <span className="text-slate-400 text-xs font-semibold">FINAL</span>}
                {isUpcoming && <span className="text-slate-500 text-xs">{game.status?.type?.shortDetail}</span>}
              </div>
            );
          })}
        </div>
