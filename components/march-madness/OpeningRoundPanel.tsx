...
export function OpeningRoundPanel({
  ...
  // Group Opening Round games by region (important for 76‑team format)
  const grouped: Record<string, TournamentGame[]> = {};
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {regionOrder.map((region) => (
          <div key={region} className="space-y-4">
            <h3 className="text-lg font-semibold text-center text-gray-200 uppercase tracking-wide">
              {region}
            </h3>

            {grouped[region].map((g) => {
